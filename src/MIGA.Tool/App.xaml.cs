...
namespace MIGA.Tool
{
  ...
  public partial class App
  {
    ...
    public static string GetRepositoryPath()
    {
      ...
      MIGA.FileSystem.FileSystem.Local.Directory.Ensure(path);
      return path;
    }
    #endregion
    ...
    protected override void OnStartup([CanBeNull] StartupEventArgs e)
    {
      ...
      if (!ApplicationManager.IsDev && !ApplicationManager.IsDebugging)
      {
        try
        {
          ...
          CoreApp.RunApp("rundll32.exe", $"dfshim.dll,ShOpenVerbApplication http://dl.sitecore.net/updater/{prefix}sim/MIGA.Tool{suffix}.application");
        }
        catch (Exception ex)
        {
          Log.Error(ex, "Error connecting to MIGA auto-updater.");
        }
      }
    ...
    {
      var _logger = new SitecoreLogger(Microsoft.Extensions.Logging.LogLevel.Debug);
    ...
    private static void LoadIocResourcesForSolr()
    {
      ...
      if (!Directory.Exists("IOC_Containers"))
      {
        ...
        ApplicationManager.GetEmbeddedFile("IOC_Containers.zip", "MIGA.Pipelines", "IOC_Containers");
      }
    }
    ...
    private static bool CheckPermissions()
    {
      ...
      {
        throw new InvalidOperationException("MIGA requires administrator permissions to operate. Relaunch Visual Studio with elevated permissions to debug MIGA.");
      }
    ...
    private static Base.Profiles.Profile DetectProfile()
    {
      ...
      Action action = delegate
      {
        try
        {
          ...
          if (!MIGA.FileSystem.FileSystem.Local.File.Exists(lic))
          {
            MIGA.FileSystem.FileSystem.Local.File.Copy(instance.LicencePath, lic);
          }
