...
using MIGA.Sitecore9Installer.Events;
  ...
  public class Tasker
  {
    ...
    public Tasker(string unInstallParamsPath) : this()
    {
      ...
      foreach (string paramsFile in uninstallTasksNames)
      {
        ...
        {
          Tasks.Task t = this.Tasks.FirstOrDefault(task => task.Name == taskName);
          string data = reader.ReadToEnd();
    ...
    private Tasks.Task CreateTask(int order, JProperty taskDescriptor)
    {
    ...
    public List<Tasks.Task> Tasks { get; } = new List<Tasks.Task>();
    ...
    public bool UnInstall
    {
      ...
      {
        unInstall = value;
        foreach (Tasks.Task task in Tasks) task.UnInstall = value;
      }
    }
    ...
    {
      foreach (Tasks.Task t in this.Tasks.Where(t => t.ExecutionOrder < 0))
      {
    ...
    private void LoadTasks()
    {
      ...
      {        
        ...
        Tasks.Task t = this.CreateTask(order, param);
