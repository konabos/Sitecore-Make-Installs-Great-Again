...
#region Methods
  ...
  {
    Log.Info(@""[MIGA] " + InstallPackageFileName + @": "" + message, this);
...
#region Methods
  ...
  protected override void OnInitComplete(EventArgs e)
  {
    ...
    {
      this.UpdateStatus(""Started: actions performing"");
      Job job = new Job(new JobOptions(""PostInstallActions"", ""MIGA"", ""shell"", this, ""Run""));
      job.Start();
      while (!job.IsDone)
      {
        Thread.Sleep(1000);
  ...
  private void UpdateStatus(string message)
  {
    ...
    var installedTemp = this.Server.MapPath(Path.Combine(tempFolderPath, ""MIGA.status""));
    File.WriteAllText(installedTemp, message);
    Log.Info(@""[MIGA] " + InstallPackageFileName + @": "" + message, this);
  }
...
#region Methods 
  ...
  protected override void OnInitComplete(EventArgs e)
  {
    ...
    Log.Info(@""[MIGA] Status.aspx: "" + message, this);
    this.Response.Write(message);
  }
